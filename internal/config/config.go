...
type Config struct {
	...
	ForwardPassTimeout time.Duration
	MaxBufferBytes     int
...
func Load() *Config {
	return &Config{
		...
		RequestTimeout:     getDuration("PG_REQUEST_TIMEOUT", 10*time.Minute),
		RedisTimeout:       getDuration("PG_REDIS_TIMEOUT", 8*time.Millisecond),
		ForwardPassTimeout: getDuration("PG_FORWARD_PASS_TIMEOUT", 10*time.Second),
